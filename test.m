...
for k = 1:size
    S_t0(k) = data(transitions(k),4);
    E_lower(k) = data(transitions(k),10); 
    n(k) = data(transitions(k),8);              % Temperature dependent coefficient for air broadened HWHM(Lorentzian)
    v0(k) = data(transitions(k),3);             % Transition wavenumber
    v(k,:) = linspace(6280,6289,10000); 
    gammaAir(k) = data(transitions(k),6);       % Air broadened HWHM 
    gammaSelf(k) = data(transitions(k),7);      % Self broadened HWHM
