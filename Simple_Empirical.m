...
for n = 1:isoSize
    for k = 1:dataSize(n)
        ...
        .* gammaV{n}(k) ) .* exp( (-1*log(2).*(v{n}(k,:)-v0{n}(k)).^2 ) ./ (gammaV{n}(k).^2) ) ;
        ...
        simpleApprox{n}(k,:) = (P*concentration*pLength./sigmaG{n}(k).*tempLineStrength{n}(k).*sqrt(log(2)/pi).*(approx{n}(k,:)))./18;
    end
end
