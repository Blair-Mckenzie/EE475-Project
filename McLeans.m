...
function mcleans = Mcleans(isoSize,dataSize,gammaG,v,v0,P,pShift,Y...
    ,S_t0,Q_tref,Q_t,c2,E_lower,T,T0,concentration,pLength)
...
for n = 1:isoSize
    for k = 1:dataSize(n)
        ...
        X{n}(k,:) = (2*sqrt(log(2))./gammaG{n}(k)).*(v{n}(k,:)-...
            v0{n}(k)')-(P.*pShift{n}(k));  
        
        for index = 1:4
            Vxy{n}(:,index,k) = ((C(index).*(Y{n}(k)-A(index)))+...
                D(index).*(X{n}(k,:)-B(index))) ./ ((Y{n}(k)-A(index))...
                .^2 + (X{n}(k,:)-B(index)).^2);
        end
        ...
        % Final aborsption for McLean's approximation
        mcleans{n}(k,:) =  2*P*concentration*pLength./gammaG{n}(k).*...
            tempLineStrength{n}(k).*sqrt(log(2)/pi).*sum(Vxy{n}(:,:,k).');
    end
end
